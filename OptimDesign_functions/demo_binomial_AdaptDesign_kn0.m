...
for t=1:p
    ...
    for i=1:length(gridu)
        [e(i)] = VBA_designEfficiency(f_fname,g_fname,dim,opt,gridu(i),'parameters');
    end
    ind = find(e==max(e));
    ustar = gridu(ind);
    if optimDesign
...
%     if ~isequal(g_fname([],phi,u(t),[]),g_fname2([],phi,u(t),[]))
%         error
%     end
...
%             y(1:t),u(1:t),[],g_fname,dim,options),...
%             VBA_NLStateSpaceModel(...
%             y(1:t),u(1:t),[],g_fname2,dim,options))
%         error
%     end
...
gxhat = g_fname([],posterior.muPhi,sort(u),[]);
vy = diag(vy);
